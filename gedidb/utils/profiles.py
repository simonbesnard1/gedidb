...
class GEDIVerticalProfiler:
    ...
    def read_pgap_theta_z(
        ...
        flat_pgap_prof = np.asarray(beam_obj["pgap_theta_z"][first:last_end], dtype=self.out_dtype)
        ...
        pgap_theta = np.asarray(beam_obj["pgap_theta"][start:finish], dtype=self.out_dtype)  # (N,)
        ...
        self._waveform_1d_to_2d(local_start_indices, counts, flat_pgap_prof,
                                out_pgap_profile, start_offset=start_offset)

        # Height-above-ground per bin (M, N)
        h0 = np.asarray(beam_obj["geolocation/height_bin0"][start:finish], dtype=np.float64)      # top
        hL = np.asarray(beam_obj["geolocation/height_lastbin"][start:finish], dtype=np.float64)   # bottom
        denom = np.maximum(counts - 1, 1)  # guard c==1
        v = (h0 - hL) / denom  # per-shot spacing

        bin_idx = np.arange(max_count, dtype=np.float64)[:, None]  # (M,1)
        out_height = (h0[None, :] - bin_idx * v[None, :] + start_offset * v[None, :]).astype(self.out_dtype)
    ...
    def compute_profiles(
        ...
        if G.ndim == 0: G = G[None]
        if O.ndim == 0: O = O[None]
        while G.ndim < pgap.ndim - 1: G = G[None, ...]
        while O.ndim < pgap.ndim - 1: O = O[None, ...]
        ...
        if z_in.ndim == 1:
            ...
            if z_in.shape != pai_z.shape:
                raise ValueError("height (n_shots, nz) must match pgap/pai shapes (n_shots, nz).")
        ...
        if z_in.ndim == 1:
            ...
            height_2d = np.broadcast_to(z_in, (n_shots, z_in.shape[0])).astype(np.float64, copy=False)
        else:
            height_2d = z_in
        ...
        cov_rh, pai_rh, pavd_rh, height_rh, H = self.resample_profiles_to_rh101_per_shot(
            height_masked, cover_m, pai_m, pavd_m,
    ...
    def resample_profiles_to_rh101_per_shot(
        ...
        rh98: Optional[np.ndarray] = None,  # (n_shots,), *height above ground* if provided
        *,
        ...
        def _interp_to_rh(rh_axis, r, y, *, fill="edge", dtype=np.float64):
            ...
            y = y[idx]
            if r.size == 1:
                return np.full_like(rh_axis, y[0] if fill == "edge" else np.nan, dtype=dtype)
            left_val, right_val = (y[0], y[-1]) if fill == "edge" else (np.nan, np.nan)
            return np.interp(rh_axis, r, y, left=left_val, right=right_val).astype(dtype, copy=False)
        ...
        height_rh[valid] = (H[valid, None] * (rh_axis[None, :] / 100.0)).astype(out_dtype, copy=False)
