...
class TestGEDIProvider(unittest.TestCase):
    ...
    def test_get_data_with_time_range(self):
        ...
        end_time = "2020-06-09"
        result = self.gedi_provider.get_data(variables, start_time=start_time, end_time=end_time)
        self.assertIsNotNone(result, "Result should not be None")
        self.assertTrue(isinstance(result, Dataset), "Result should be an xarray Dataset")
    ...
    def test_get_data_with_quality_filters(self):
        ...
        self.assertTrue(isinstance(unfiltered_result, Dataset), "Unfiltered result should be an xarray Dataset")
        ...
        self.assertTrue(isinstance(filtered_result, Dataset), "Filtered result should be an xarray Dataset")
    ...
    def test_get_data_with_different_return_types(self):
        ...
        self.assertTrue(isinstance(result_xarray, Dataset), "Result should be an xarray Dataset")

        # Test pandas DataFrame return type
        result_df = self.gedi_provider.get_data(variables, return_type="dataframe")
        self.assertTrue(isinstance(result_df, DataFrame), "Result should be a pandas DataFrame")
