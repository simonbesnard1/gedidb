...
class EarthDataAuthenticator:
    ...
    def __init__(self):
        ...
        logger.info("Authenticator initialized with storage directory set to %s", self.earth_data_dir)

    def authenticate(self):
        """Main method to manage authentication and cookies."""
        if self.cookie_file.exists():
            logger.info("Authentication cookie file found at %s. Skipping authentication.", self.cookie_file)
        else:
    ...
    def _credentials_in_netrc(self) -> bool:
        ...
        try:
            ...
            logger.warning(".netrc file not found at %s; will create a new one.", self.netrc_file)
            return False
    ...
    def _add_netrc_credentials(self):
        ...
        try:
            ...
            logger.info("Credentials added to .netrc file with secure permissions at %s.", self.netrc_file)
        except OSError as e:
            logger.error("Error adding credentials to .netrc file at %s: %s", self.netrc_file, e)
            raise
    ...
    def _fetch_earthdata_cookies(self):
        ...
        try:
            ...
            logger.info("Attempting to fetch Earthdata cookies and save to %s", self.cookie_file)
            subprocess.run(
                [
                    "wget",
            ...
            logger.info("Earthdata cookies successfully fetched and saved to %s.", self.cookie_file)
